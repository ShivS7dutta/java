...
public class AnagramGroups {
    ...
    public static void main(String[] args) {
        ...
        for (String word : anagramWords) {
            ...
            /*
             Sorting: We convert each word to a character array, sort it, and then convert it back to a string (sortedWord). This sorted version of the word serves as the key in the HashMap where words that are anagrams will have the same key.

        HashMap for Grouping: We use a HashMap<String, List<String>> where the key is the sorted word and the value is a list of words (anagrams) that have the same sorted form.

      Storing and Grouping:

       If the sorted word is not already in the map, we use putIfAbsent to initialize a new list.
       We then add the word to the list associated with the sorted word in the map.
       Output:

      After grouping the words, we iterate through the values of the HashMap (which are lists of anagrams) and add the first word from each list to the result.
             */
